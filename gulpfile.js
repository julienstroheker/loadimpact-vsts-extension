...
gulp.task('compileTasks', ['clean'], function (cb) {
    try {
        ...
        getNpmExternal('isomorphic-fetch');
    }
    catch (err) {
    ...
    var tasksPath = path.join(__dirname, 'LoadImpact', '**/*.ts');
    return gulp.src([tasksPath, 'definitions/*.d.ts'])
        .pipe(tsc())
        .pipe(gulp.dest(path.join(__dirname, 'LoadImpact')));
});
...
gulp.task('build', ['locCommon'], function () {
    ...
    return gulp.src(path.join(__dirname, 'LoadImpact', '**/task.json'))
